...
**看门狗 + 开机自启（v3.10.0+）**：守护进程因任何原因死掉（系统重启、Python 崩溃、被 kill）都会被自动拉起。两条兜底通路：

- Web UI 端：每次访问 `/api/monitor/status`（前端每 10s 轮询一次）发现 `enabled && !running` 立刻 spawn
- 开机自启：启用监控时自动注册 Windows 任务计划程序任务 `CCS-UsageMonitor`（用户登录触发，无需管理员），执行 `ccs monitor revive` 拉起守护

相关 CLI：

```bash
ccs monitor status          # 查守护 pid / 运行时长 / 最近日志
ccs monitor enable          # 启用守护并注册开机自启
ccs monitor disable         # 停止并移除自启
ccs monitor revive          # 守护掉了就拉起来（任务计划程序登录时调用）
ccs autostart status        # 查任务计划任务的注册状态、上次运行时间
ccs autostart install       # 单独注册自启（不动守护开关）
ccs autostart remove        # 单独移除自启
```

仅 Windows 支持任务计划程序集成。**已知限制**：Windows 睡眠/休眠期间 Python 进程冻结，醒来后继续跑但中间不会查询用量；若睡眠中跨过了 5h 撞墙时刻仍会漏切（OS 层面限制，强制唤醒电脑体验更差，未启用）。
...
| `~/.ccs/autostart-revive.bat` | 任务计划程序调用的胶水脚本（v3.10.0+，启用守护时生成） |
...
- **v3.10.0**：守护进程看门狗 + 开机自启
  - 修复：v3.9.0 改成 Web UI 独占切换权后，守护进程一旦因系统重启/崩溃/被 kill 死掉就再起不来，用户撞到 100% 也不会切（实测案例：4 天没拉起，用户手动切完才发现）
  - 新增 `src/monitor.js` `revive()` 函数：开关开 + 进程没活就 spawn，否则 noop
  - Web UI `/api/monitor/status` 接口每次都先调 `revive()`，前端 10s 轮询自带兜底
  - 新增 `src/autostart.js`：用 Windows 任务计划程序 `schtasks` 在用户登录时执行 `ccs monitor revive`；用 `~/.ccs/autostart-revive.bat` 胶水脚本规避 `/TR` 引号转义在 Node spawn 下的坑
  - `monitor.enable()` / `disable()` 内部联动 `autostart.install()` / `remove()`，UI 开关一键生效
  - 新增 CLI：`ccs monitor status|enable|disable|revive`、`ccs autostart status|install|remove`
  - UI 文案修正：原「○ 未运行（5h < 90% 时守护不启动）」是 v3.9.0 前旧策略的残留，改成红色异常告警「⚠ 已启用但守护未运行（自动重启失败，请查看日志）」
