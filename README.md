...
Windows / macOS CLI 工具，通过备份和还原 `~/.claude/.credentials.json` 及 `~/.claude.json` 中的账号字段，实现 Claude Code 多账号一键切换。支持 OAuth 和 API Key 两种账号类型，提供命令行和 Web UI 两种操作方式。
...
Windows 安装后会自动在桌面创建「CCS 管理界面」快捷方式，双击即可启动 Web UI（自动杀掉旧进程再重启）。
...
启动后自动打开浏览器，访问 `http://127.0.0.1:7899`。支持：

- 查看所有账号状态并一键切换
- 导入 OAuth 账号（读取当前登录的 credentials）
- 导入 API Key 账号（写入 `~/.claude/settings.json` 的 env 字段）
- 编辑、删除账号

**Windows**：安装后桌面快捷方式双击即可，无需手动启动终端。

**macOS**：运行 `ccs web` 会自动打开浏览器。
...
`scripts/statusline-command.sh` 是 Claude Code 状态栏脚本，在每次会话结束时输出三行信息：
