...
- 第三行：OAuth 姓名、邮箱、套餐（5 分钟缓存）；**自动切换后显示红色提示「⚠ 已切到 X（HH:MM），重启 Claude Code 生效」**，重启后自动消失

**自动切换**（v3.8.2+）：

- 5h ≥ 99% 时自动切到用量最低的可用 OAuth 账号
- 关闭开关：`touch ~/.ccs/auto-switch.disabled`
- 日志：`~/.ccs/auto-switch.log`

**用量监控守护进程**（v3.8.7+）：

状态栏脚本在 active 5h ≥ 90% 时 spawn 单例守护进程，独立轮询用量、在撞墙前主动切换，不再依赖"说了话 Claude 答完才检测"：

- 90–95%：每 60s 查一次
- 96–98%：每 10s 查一次
- ≥ 99% 或 API 返回 429：立即切换，守护退出
- 用量降回 90% 以下（切换成功或自然 reset）：守护退出
- 关闭守护：`touch ~/.ccs/usage-monitor.disabled`（不影响状态栏 tick 触发的切换）

在 Web UI 的「Claude Code 状态栏」区块点「安装」即可：同时复制 `statusline-command.sh`、`auto_switch_core.py`、`usage_monitor.py` 到 `~/.claude/`，并写入 `~/.claude/settings.json` 的 `statusLine` 字段。卸载同理一键完成。
...
| `~/.ccs/account-usage.json` | 账号用量缓存表（每次状态栏 tick 更新 active 条目） |
| `~/.ccs/auto-switch.log` | 自动切换 + 用量监控守护进程日志 |
| `~/.ccs/auto-switch.disabled` | 存在时关闭自动切换（touch 创建即可） |
| `~/.ccs/usage-monitor.pid` | 用量监控守护进程 PID 文件 |
| `~/.ccs/usage-monitor.disabled` | 存在时关闭守护进程（不影响状态栏 tick 触发的切换） |
| `~/.ccs/last-switch.json` | 最近一次自动切换记录（供状态栏显示重启提示） |
| `~/.claude/auto_switch_core.py` | 切换决策核心模块（与状态栏脚本同目录，安装时复制） |
| `~/.claude/usage_monitor.py` | 用量监控守护进程（安装时复制） |
